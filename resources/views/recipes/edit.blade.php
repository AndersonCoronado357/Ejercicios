...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-lg p-6">
        <div class="mb-6">
            <a href="{{ route('recipes.show', $recipe) }}" class="text-gray-600 hover:text-gray-800">
                <i class="fas fa-arrow-left mr-2"></i>Volver a la Receta
            </a>
        </div>
        ...
        <form action="{{ route('recipes.update', $recipe) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="3" required
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                          placeholder="Describe brevemente la receta...">{{ old('description', $recipe->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid md:grid-cols-3 gap-4 mb-4">
                ...
                <div>
                    ...
                    <select name="difficulty" required
                        ...
                        @foreach(\App\Models\Recipe::getDifficulties() as $key => $diff)
                            <option value="{{ $key }}" {{ old('difficulty', $recipe->difficulty) == $key ? 'selected' : '' }}>
                                {{ $diff['name'] }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="servings" value="{{ old('servings', $recipe->servings) }}" min="1" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500">
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" name="prep_time" value="{{ old('prep_time', $recipe->prep_time) }}" min="1" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500">
                </div>

                <div>
                    <label class="block text-gray-700 text-sm font-bold mb-2">Tiempo de Cocción (min)</label>
                    <input type="number" name="cook_time" value="{{ old('cook_time', $recipe->cook_time) }}" min="0" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500">
                </div>
            </div>
            ...
            <div class="mb-6">
                <label class="flex items-center cursor-pointer">
                    <input type="checkbox" name="is_favorite" value="1" {{ old('is_favorite', $recipe->is_favorite) ? 'checked' : '' }}
            ...
            <div class="flex gap-4">
                ...
                <a href="{{ route('recipes.show', $recipe) }}" class="flex-1 py-3 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition font-bold text-center">
                    Cancelar
                </a>
            </div>
        </form>
