...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-lg p-6">
        ...
        <h1 class="text-3xl font-bold text-gray-800 mb-6">Editar Nota</h1>
        ...
        <form action="{{ route('notes.update', $note) }}" method="POST">
                ...
                <div>
                    ...
                    <select name="color" required
                        ...
                        @foreach(\App\Models\Note::getColors() as $key => $color)
                            <option value="{{ $key }}" {{ old('color', $note->color) == $key ? 'selected' : '' }}>
                                {{ $color['name'] }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="flex gap-4">
                <button type="submit" class="flex-1 py-3 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition font-bold">
                    <i class="fas fa-save mr-2"></i>Actualizar Nota
                </button>
